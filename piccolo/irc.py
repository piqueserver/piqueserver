...
from piccolo.commands import command, restrict
...
class IRCClientFactory(protocol.ClientFactory):
    ...
    def __init__(self, server, config):
        ...
        self.nickname = config.get('nickname',
                                   'piccolo%s' % random.randrange(0, 99))
        self.username = config.get('username', 'piccolo')
        self.realname = config.get('realname', server.name)
        self.channel = config.get('channel', "#piccolo.bots").lower()
