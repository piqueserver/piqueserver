    ...
    class QueryProtocol(protocol):
        ...
        def handleQuery(self, challenge):
            ...
            for p in self.players.values():
                players.append({'score': p.kills, 'ping': p.latency, 'name': makeValid(
                    p.name), 'team': getTeamId(p.team.id)})
        ...
        def receive_callback(self, address, data):
            if data and data.startswith(STATUS_REQUEST):
                ...
                binmsg = '\n'.join([STATUS_REPLY, msg.encode(
                    'ascii', 'ignore'), plr.encode('ascii', 'ignore')])
